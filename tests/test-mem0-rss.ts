...
function printRss(label: string, before: NodeJS.MemoryUsage, after: NodeJS.MemoryUsage) {
  ...
  console.log(`\n=== ${label} ===`);
  console.log(`  RSS delta:       ${(rss / 1024).toFixed(1)} KB (${(rss / 1024 / 1024).toFixed(2)} MB)`);
  console.log(`  Heap used delta: ${(heap / 1024).toFixed(1)} KB (${(heap / 1024 / 1024).toFixed(2)} MB)`);
...
async function main() {
  ...
  console.log(`  Without mem0: ${(rssNoMem0 / 1024).toFixed(1)} KB`);
  console.log(`  With mem0:    ${(rssWithMem0 / 1024).toFixed(1)} KB`);
  console.log(`  Overhead:     ${((rssWithMem0 - rssNoMem0) / 1024).toFixed(1)} KB`);
}
